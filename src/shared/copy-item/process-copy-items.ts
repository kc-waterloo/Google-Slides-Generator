...
import { setBold_ } from "../shape/set-bold";
import { setTextForegroundColor_ } from "../shape/set-text-foreground-color";
		...
		if (copyItem.originalPageElement) {
			...
			if (copyItem.newPageElement) {
				...
				if (newShape) {
					...
					if (copyItem.actions.newBorderColor !== undefined) {
						setBorderColor_({
							shape: newShape,
							newColor: copyItem.actions.newBorderColor,
						});
					}
				}
			}
